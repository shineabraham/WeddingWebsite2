...
export default function HUD({ ceremonyDate, coordinates }: HUDProps) {
  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed inset-0 z-[100] hidden p-6 md:block">
        {/* Top-left: Countdown */}
        <motion.div
      ...
      {/* Mobile: minimal bottom bar with audio only */}
      <motion.div
        className="pointer-events-none fixed bottom-0 left-0 right-0 z-[100] flex justify-end p-3 md:hidden"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 3, duration: 0.8 }}
      >
        <button
          className="pointer-events-auto flex items-center gap-2 border border-gold/20 bg-obsidian/90 px-3 py-2 font-body text-[9px] uppercase tracking-ultrawide text-gold backdrop-blur-sm transition-colors hover:border-gold/40 focus-visible:outline-none active:scale-95"
        ...
        >
          ...
          {isMuted ? "Sound" : "Playing"}
        </button>
      </motion.div>

      {/* Desktop audio button */}
      <motion.div
        className="pointer-events-none fixed bottom-6 right-6 z-[100] hidden md:block"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 3, duration: 0.8 }}
      >
        <button
          className="pointer-events-auto flex items-center gap-2 border border-gold/20 bg-obsidian/80 px-3 py-1.5 font-body text-[9px] uppercase tracking-ultrawide text-gold backdrop-blur-sm transition-colors hover:border-gold/40 focus-visible:outline-none active:scale-95"
          onClick={() => setIsMuted(!isMuted)}
        >
          <svg
            width="14"
